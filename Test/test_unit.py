...
from website_monitor.main import (
    check_file,
    filter_element,
    get_commented_data,
    get_csv_data,
...
class TestFilterElement(unittest.TestCase):
    ...
    def test_filter_no_element(self):
        self.assertEqual(filter_element(self.fake_html, ""), self.fake_html)
...
class TestGetCommentedData(unittest.TestCase):
    def test_no_comment(self):
        self.assertEqual(
            get_commented_data(pathlib.Path(__file__).with_name("website_data.csv")), {}
        )
...
class TestGetCsvData(unittest.TestCase):
    ...
    def test_get_csv_data_with_comments(self):
        self.assertEqual(
            get_csv_data(pathlib.Path(__file__).with_name("website_data_comment.csv")),
            {
                "https://www.provincia.brescia.it/istituzionale/concorsi": {
                    "hash": "714120cf1abda6bac178b4e23c9c2fbacc93b87c5"
                    "b9b560d90af4e9904dc3be4",
    ...
    def test_get_initial_csv_data(self):
        self.assertEqual(
            get_csv_data(pathlib.Path(__file__).with_name("website_data_initial.csv")),
            {
                "https://apple.com": {
                    "hash": None,
                    "filter": "test-element",
...
class TestGetOutputChannel(unittest.TestCase):
    ...
    def test_get_output_channel_args(self):
        ...
        self.assertEqual(
            telegram_tuple, (telegram.Bot(token=self.fake_token), "chat_id")
        )
...
class TestSendOutput(unittest.TestCase):
    ...
    def test_send_output_to_cli_print(self, mock_print):
        ...
        self.assertEqual(
            mock_print.getvalue(), "⏬ Check results ⏬\nfirst\n\nsecond\n"
        )
    ...
    def test_send_output_to_telegram(self, mock_bot):
        send_output(
            ["first", "second"],
            (
                telegram.Bot(token="0123456789:AAHkOz6994U2SilZ3Z4cba6aZaZabcd38Z8"),
                "chat_id",
            ),
        )
...
class TestOpenWebsite(unittest.TestCase):
    ...
    def test_open_website(self):
        responses.add(
            responses.GET, "http://www.test.com", body=b"Hello world!", status=200
        )
